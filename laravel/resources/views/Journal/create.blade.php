...
@section('content')
    <div class="min-h-screen bg-gradient-to-br from-slate-50 via-violet-50/30 to-slate-50">
                ...
                <div class="flex items-center justify-between">
                    <div>
                        <nav class="flex items-center gap-2 text-sm text-slate-500 mb-3">
                            ...
                            <span class="text-slate-700">{{ $journal->title ?? 'New Entry' }}</span>
                        </nav>
                        <h1 class="font-display text-3xl text-slate-900">
                            {{ $journal->exists ? 'Edit Journal Entry' : 'Create Your Journal Entry' }}
                        </h1>
                        <p class="mt-2 text-slate-500">
                            {{ $journal->exists ? 'Update your thoughts, ideas, and reflections.' : 'Capture your thoughts, ideas, and memories.' }}
                        </p>
                    </div>
                    <div class="hidden md:flex items-center gap-3">
                        <div class="flex items-center gap-2 px-4 py-2 bg-violet-50 rounded-full">
                            ...
                            <span class="text-sm font-medium text-violet-700">{{ $journal->date?->format('F j, Y') ?? now()->format('F j, Y') }}</span>
                        </div>
                        @if($journal->exists)
                            <a href="{{ route('journal.show', $journal->id) }}" class="text-slate-500 hover:text-violet-600 transition-colors">
        ...
        <div class="max-w-4xl mx-auto px-6 py-10">
            {{-- Tips Card (only show for new entries) --}}
